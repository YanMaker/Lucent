...
let newTabUrlBar = document.querySelector("#newTabUrlBar input");
...
urlBar.addEventListener("focusin", () => {
    ...
    forms[0].classList.add("focus");
});
urlBar.addEventListener("focusout", () => {
    urlRecommendations.style.display = "none";
    forms[0].classList.remove("focus");
});
newTabUrlBar.addEventListener("focusin", () => {
    newTabUrlBar.select();
});
...
forms.forEach((form) => {
    form.onsubmit = function (event) {
        ...
        document
            .querySelector(":root")
            .style.setProperty("--urlBarBorderRadius", "5px");
        urlBar.blur();
    };
});
...
function newTab(url) {
    ...
    favicon.src = "img/lucent_icon_basic.png";
    ...
    function onClick() {
        ...
        if (!webview.src.includes("dummypage.html")) {
    ...
    webview.addEventListener("did-start-loading", function () {
        favicon.src = "img/loading_favicon.svg";
    });
...
window.api.handle(
    ...
    () =>
        function () {
            document.querySelector("#tabs").style.marginLeft = "75px";
            document.querySelector("#tabs").style.maxWidth =
                "calc(100% - 115px)";
        }
);
...
function home() {
    document.querySelector("webview.active").remove();
};
